...
# Завдання 3.6
Дослідження обмеження стеку та фіксованої пам’яті

У цьому завданні потрібно було дослідити два типи обмежень ресурсів у Linux: максимальний розмір стеку процесу (max stack segment size) та обмеження на фіксовану пам’ять (locked memory). Для цього було написано дві невеликі програми на мові C.

## Дослідження обмеження стеку

Спочатку було створено програму stack_demo.c, яка використовує рекурсію. Рекурсивні виклики активно використовують стек, тому така програма швидко заповнює доступну стекову пам’ять.

Програму було скомпільовано за допомогою компілятора gcc.
```
gcc stack_demo.c -o stack_demo
```
Після цього було встановлено обмеження на розмір стеку:
```
ulimit -s 64
```
Це означає, що програмі доступно лише 64 КБ стекової пам’яті.

Для перевірки значення було виконано команду:
```
ulimit -s
```
...
./stack_demo
```
програма почала виводити глибину рекурсії. Через деякий час стек переповнився і система завершила виконання програми.

У терміналі з’явилось повідомлення:
```
Segmentation fault (core dumped)
```
Результат виконання

![Компіляція та встановлення stack limit](screenshots/1comp+64.png)

![Переповнення стеку](screenshots/1res.png)

Це означає, що програма використала весь доступний стек і система завершила її роботу.

## Дослідження locked memory

У другій частині завдання було досліджено обмеження на фіксовану пам’ять. Для цього була написана програма lock_demo.c, яка виділяє пам’ять і намагається закріпити її в оперативній пам’яті за допомогою функції mlock().

Спочатку програму було скомпільовано:
```
gcc lock_demo.c -o lock_demo
```
Далі було перевірено поточне значення обмеження:
```
ulimit -l
```
Після цього було встановлено нове обмеження:
```
ulimit -l 64
```
Це означає, що процес може зафіксувати у пам’яті не більше 64 КБ.
...
```
Це означає, що програма намагалася зафіксувати більше пам’яті, ніж дозволяє встановлений ліміт.

Результат виконання

![Locked memory limit](screenshots/2.png)

### Висновок

У цьому завданні було досліджено два типи обмежень ресурсів у Linux. У першій частині було показано, що при малому значенні ulimit -s рекурсивна програма швидко переповнює стек і завершується з помилкою. У другій частині було перевірено обмеження ulimit -l, яке визначає максимальний обсяг пам’яті, що може бути зафіксований у RAM. При спробі перевищити цей ліміт система не дозволяє виконати операцію mlock, що підтверджує роботу механізму обмеження ресурсів.
