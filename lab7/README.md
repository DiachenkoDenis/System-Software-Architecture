...
## Завдання 9 
У цьому завданні було реалізовано вимір часу виконання певного фрагмента коду. Для цього використовується функція clock_gettime() з параметром CLOCK_MONOTONIC.

Програма фіксує час до виконання циклу і після нього, після чого обчислює різницю у мілісекундах. Це дозволяє оцінити продуктивність виконання коду.

### Код програми
Код програми розміщено у файлі: code/task9.c
...
Після запуску програма вивела значення змінної та час виконання у мілісекундах. Це означає, що вимірювання часу було виконано правильно.

На скріншоті видно конкретне значення часу, що залежить від продуктивності системи.

## Завдання 10  
У цьому завданні було реалізовано генерацію випадкових чисел з плаваючою комою у різних діапазонах. Для цього використовується функція rand() та ініціалізація генератора через srand(time(NULL)).

Програма генерує числа у діапазоні від 0 до 1, а також у діапазоні від 0 до заданого користувачем числа. Це демонструє роботу з випадковими значеннями.

### Код програми
Код програми розміщено у файлі: code/task10.c
...
Після запуску програма згенерувала випадкові числа у заданих діапазонах. Значення змінюються при кожному запуску, що підтверджує правильну ініціалізацію генератора.

На скріншоті видно набір випадкових чисел, що відповідає умовам завдання.

## Завдання по варіантам (варіант 6)  
У цьому завданні було реалізовано програму, яка змінює права доступу до файлів залежно від кількості їх використання. Оскільки в Linux немає прямого способу отримати таку інформацію, вона була змодельована через файл usage.txt.

Програма зчитує кількість використань для кожного файлу і змінює права доступу через chmod(). Якщо значення велике - додається право читання, якщо мале - воно прибирається.

### Код програми
Код програми розміщено у файлі: code/task_variant6.c

### Компіляція програми
gcc -Wall task_variant6.c -o task_variant6.c
### Запуск програми
./task_variant6.c
### Результати виконання
![Результат](screenshots/task_variant6.png)

Після запуску програма змінила права доступу до файлів залежно від їх “використання”. Це підтверджує, що логіка умовної зміни прав працює правильно.

На скріншоті видно повідомлення про зміну прав доступу, що підтверджує виконання завдання.

### Висновок
У ході виконання лабораторної роботи було досліджено основні механізми роботи з файловою системою Linux та системними викликами. Було реалізовано аналоги стандартних утиліт, що дозволило краще зрозуміти їх внутрішню логіку.

Отримані результати показують, що навіть базовими засобами мови C можна реалізувати складні системні інструменти. Це дає практичне розуміння роботи операційної системи Linux на низькому рівні.
